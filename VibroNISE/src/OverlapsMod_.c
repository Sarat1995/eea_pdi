...
// Change Huang-Rhys factor name!


void BuildVOverlaps(RealType **VOverlaps, const RealType *HuangRhys, const int VTotal, const int Embarrassing)
{
	int PriEStateCount, SecEStateCount, PriVCount, SecVCount;
	RealType *HRFactors = malloc(5 * 5 * sizeof(RealType));
	
	for (PriEStateCount = 0; PriEStateCount < 5; PriEStateCount++)
		for(SecEStateCount = 0; SecEStateCount < 5; SecEStateCount++)
			HRFactors(PriEStateCount * 5 + SecEStateCount) = HuangRhys[PriEStateCount] - HuangRhys[SecEStateCount];
	
	*VOverlaps = malloc(IntPow2(5 * VTotal) * sizeof(RealType));
	
	for (PriEStateCount = 0; PriEStateCount < 5; PriEStateCount++)
		for(SecEStateCount = 0; SecEStateCount < 5; SecEStateCount++) {
			VShift = HuangRhys[PriEStateCount] - HuangRhys[SecEStateCount];
			HRFactor = VShift * VShift;
			Offset = (PriEStateCount * 5 + SecEStateCount) * VTotal * VTotal;
			for (PriVCount = 0; PriVCount < VTotal; PriVCount++)
				for (SecVCount = 0; SecVCount < VTotal; SecVCount++)
					(*VOverlaps)[Offset + PriVCount * VTotal + SecVCount] = Overlap(PriVCount, SecVCount, VShift, HRFactor);
		}
	
	if (!Embarrassing) SaveOverlaps("VOverlaps.dat", *VOverlaps, 5 * VTotal);
